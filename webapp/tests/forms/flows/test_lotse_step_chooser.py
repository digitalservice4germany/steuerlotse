...
class TestStepSelectStmind:
    def test_prev_step_is_correct(self, new_test_request_context):
        data = {}
    ...
    def test_if_no_select_field_set_then_next_step_is_correct(self, new_test_request_context):
        data = {}
        with new_test_request_context(stored_data=data):
            step = LotseStepChooser().get_correct_step(StepSelectStmind.name, False, ImmutableMultiDict({}))
            assert step._next_step == StepSummary
    
    def test_if_select_field_set_to_false_then_next_step_is_correct(self, new_test_request_context):
    ...
    def test_if_select_vorsorge_set_then_next_step_is_correct(self, new_test_request_context):
        data = {'stmind_select_vorsorge': True}
        with new_test_request_context(stored_data=data):
            step = LotseStepChooser().get_correct_step(StepSelectStmind.name, False, ImmutableMultiDict({}))
            assert step._next_step == StepVorsorge

    def test_if_select_ausserg_bela_set_then_next_step_is_correct(self, new_test_request_context):
        data = {'stmind_select_ausserg_bela': True}
        with new_test_request_context(stored_data=data):
            step = LotseStepChooser().get_correct_step(StepSelectStmind.name, False, ImmutableMultiDict({}))
            assert step._next_step == StepAussergBela

    def test_if_select_handwerker_set_then_next_step_is_correct(self, new_test_request_context):
        data = {'stmind_select_handwerker': True}
        with new_test_request_context(stored_data=data):
            step = LotseStepChooser().get_correct_step(StepSelectStmind.name, False, ImmutableMultiDict({}))
            assert step._next_step == StepHaushaltsnaheHandwerker

    def test_if_select_spenden_set_then_next_step_is_correct(self, new_test_request_context):
        data = {'stmind_select_spenden': True}
        with new_test_request_context(stored_data=data):
            step = LotseStepChooser().get_correct_step(StepSelectStmind.name, False, ImmutableMultiDict({}))
            assert step._next_step == StepSpenden

    def test_if_select_religion_set_then_next_step_is_correct(self, new_test_request_context):
...
class TestStepVorsorge:
    ...
    def test_set_prev_step_correctly(self, new_test_request_context):
        with new_test_request_context(stored_data=self.valid_data):
            step = LotseStepChooser().get_correct_step(StepVorsorge.name, False, ImmutableMultiDict({}))
            assert step._prev_step == StepSelectStmind

    def test_set_next_step_correctly_if_next_step_shown(self, new_test_request_context):
        with new_test_request_context(stored_data=self.valid_data_with_next_step_set):
            step = LotseStepChooser().get_correct_step(StepVorsorge.name, False, ImmutableMultiDict({}))
            assert step._next_step == StepAussergBela

    def test_redirect_to_correct_step_if_should_not_be_shown(self, new_test_request_context):
        with new_test_request_context(stored_data=self.invalid_data):
            step = LotseStepChooser().get_correct_step(StepVorsorge.name, False, ImmutableMultiDict({}))
...
class TestStepAussergBela:
    ...
    def test_if_vorsorge_shown_then_set_prev_step_correctly(self, new_test_request_context):
        with new_test_request_context(stored_data=self.valid_data_with_vorsorge_shown):
            step = LotseStepChooser().get_correct_step(StepAussergBela.name, False, ImmutableMultiDict({}))
            assert step._prev_step == StepVorsorge

    def test_if_vorsorge_not_shown_then_set_prev_step_correctly(self, new_test_request_context):
        with new_test_request_context(stored_data=self.valid_data):
            step = LotseStepChooser().get_correct_step(StepAussergBela.name, False, ImmutableMultiDict({}))
            assert step._prev_step == StepSelectStmind

    def test_if_handwerker_shown_then_set_next_step_correctly(self, new_test_request_context):
        with new_test_request_context(stored_data=self.valid_data_with_handwerker_shown):
            step = LotseStepChooser().get_correct_step(StepAussergBela.name, False, ImmutableMultiDict({}))
            assert step._next_step == StepHaushaltsnaheHandwerker

    def test_if_handwerker_not_shown_then_set_next_step_correctly(self, new_test_request_context):
        with new_test_request_context(stored_data=self.valid_data):
            step = LotseStepChooser().get_correct_step(StepAussergBela.name, False, ImmutableMultiDict({}))
            assert step._next_step == StepSummary
...
class TestStepHaushaltsnaheHandwerker:
    ...
    def test_if_ausserg_bela_shown_then_set_prev_step_correctly(self, new_test_request_context):
        with new_test_request_context(stored_data=self.valid_data_with_ausserg_bela_shown):
    ...
    def test_if_ausserg_bela_not_shown_then_set_prev_step_correctly(self, new_test_request_context):
        with new_test_request_context(stored_data=self.valid_data):
            step = LotseStepChooser().get_correct_step(StepHaushaltsnaheHandwerker.name, False, ImmutableMultiDict({}))
            assert step._prev_step == StepSelectStmind

    def test_if_gem_haushalt_not_skipped_then_set_next_step_correctly(self, new_test_request_context):
        with new_test_request_context(stored_data=self.valid_data), \
    ...
    def test_if_gem_haushalt_skipped_and_religion_shown_then_set_next_step_correctly(self, new_test_request_context):
        with new_test_request_context(stored_data=self.valid_data_with_religion_shown):
            step = LotseStepChooser().get_correct_step(StepHaushaltsnaheHandwerker.name, False, ImmutableMultiDict({}))
            assert step._next_step == StepReligion

    def test_if_gem_haushalt_skipped_and_religion_not_shown_then_set_next_step_correctly(self, new_test_request_context):
        with new_test_request_context(stored_data=self.valid_data):
            step = LotseStepChooser().get_correct_step(StepHaushaltsnaheHandwerker.name, False, ImmutableMultiDict({}))
            assert step._next_step == StepSummary
...
class TestStepGemeinsamerHaushalt:
    ...
    def test_if_religion_shown_then_set_next_step_correctly(self, new_test_request_context):
        with new_test_request_context(stored_data=self.valid_data_religion_shown):
            step = LotseStepChooser().get_correct_step(StepGemeinsamerHaushalt.name, False, ImmutableMultiDict({}))
            assert step._next_step == StepReligion

    def test_if_religion_not_shown_then_set_next_step_correctly(self, new_test_request_context):
        with new_test_request_context(stored_data=self.valid_data):
            step = LotseStepChooser().get_correct_step(StepGemeinsamerHaushalt.name, False, ImmutableMultiDict({}))
            assert step._next_step == StepSummary

    def test_if_handwerker_is_not_shown_then_redirect_to_correct_step(self, new_test_request_context):
        with new_test_request_context(stored_data=self.handwerker_not_shown_data):
            step = LotseStepChooser().get_correct_step(StepGemeinsamerHaushalt.name, False, ImmutableMultiDict({}))
            assert isinstance(step, RedirectSteuerlotseStep)
            assert step.redirection_step_name == StepSelectStmind.name

    def test_if_no_familienstand_then_redirect_to_correct_step(self, new_test_request_context):
        with new_test_request_context(stored_data=self.no_familienstand_data):
            step = LotseStepChooser().get_correct_step(StepGemeinsamerHaushalt.name, False, ImmutableMultiDict({}))
            assert isinstance(step, RedirectSteuerlotseStep)
            assert step.redirection_step_name == StepFamilienstand.name

    def test_if_zusammenveranlagung_then_redirect_to_correct_step(self, new_test_request_context):
        with new_test_request_context(stored_data=self.valid_data), \
    ...
    def test_if_einzelveranlagung_then_do_not_redirect(self, new_test_request_context):
        with new_test_request_context(stored_data=self.valid_data), \
    ...
    def test_if_no_haushaltsnahe_set_then_redirect_to_correct_step(self, new_test_request_context):
        with new_test_request_context(stored_data=self.no_haushaltsnahe_data):
            step = LotseStepChooser().get_correct_step(StepGemeinsamerHaushalt.name, False, ImmutableMultiDict({}))
            assert isinstance(step, RedirectSteuerlotseStep)
...
class TestStepReligion:
    ...
    def test_if_gem_haushalt_not_skipped_then_set_prev_step_correctly(self, new_test_request_context):
        with new_test_request_context(stored_data=self.valid_data_with_handwerker_shown), \
                patch('app.forms.steps.lotse.steuerminderungen.StepGemeinsamerHaushalt.check_precondition',
                      MagicMock(return_value=True)):
    ...
    def test_if_gem_haushalt_skipped_then_set_prev_step_correctly(self, new_test_request_context):
        with new_test_request_context(stored_data=self.valid_data_with_handwerker_shown):
            step = LotseStepChooser().get_correct_step(StepReligion.name, False, ImmutableMultiDict({}))
            assert step._prev_step == StepHaushaltsnaheHandwerker

    def test_if_handwerker_skipped_then_set_prev_step_correctly(self, new_test_request_context):
        with new_test_request_context(stored_data=self.valid_data):
            step = LotseStepChooser().get_correct_step(StepReligion.name, False, ImmutableMultiDict({}))
            assert step._prev_step == StepSelectStmind

    def test_if_spenden_shown_then_set_next_step_correctly(self, new_test_request_context):
        with new_test_request_context(stored_data=self.valid_data_with_spenden_shown):
            step = LotseStepChooser().get_correct_step(StepReligion.name, False, ImmutableMultiDict({}))
            assert step._next_step == StepSpenden

    def test_if_spenden_not_shown_then_set_next_step_correctly(self, new_test_request_context):
        with new_test_request_context(stored_data=self.valid_data):
            step = LotseStepChooser().get_correct_step(StepReligion.name, False, ImmutableMultiDict({}))
            assert step._next_step == StepSummary
...
class TestStepSpenden:
    ...
    def test_if_religion_shown_then_set_prev_step_correctly(self, new_test_request_context):
        with new_test_request_context(stored_data=self.valid_data_with_religion_shown):
            step = LotseStepChooser().get_correct_step(StepSpenden.name, False, ImmutableMultiDict({}))
            assert step._prev_step == StepReligion

    def test_if_religion_not_shown_then_set_prev_step_correctly(self, new_test_request_context):
        with new_test_request_context(stored_data=self.valid_data):
            step = LotseStepChooser().get_correct_step(StepSpenden.name, False, ImmutableMultiDict({}))
            assert step._prev_step == StepSelectStmind

    def test_set_next_step_correctly(self, new_test_request_context):
        with new_test_request_context(stored_data=self.valid_data):
            step = LotseStepChooser().get_correct_step(StepSpenden.name, False, ImmutableMultiDict({}))
            assert step._next_step == StepSummary
...
class TestStepSummary:
    ...
    def test_if_spenden_shown_then_set_prev_url_correct(self, new_test_request_context):
        with new_test_request_context(stored_data=self.valid_data_with_spenden_shown):
            step = LotseStepChooser().get_correct_step(StepSummary.name, False, ImmutableMultiDict({}))
            assert step._prev_step == StepSpenden

    def test_if_spenden_not_shown_then_set_prev_url_correct(self, new_test_request_context):
        with new_test_request_context(stored_data=self.valid_data):
            step = LotseStepChooser().get_correct_step(StepSummary.name, False, ImmutableMultiDict({}))
            assert step._prev_step == StepSelectStmind

    def test_set_next_url_correct(self, new_test_request_context):
        with new_test_request_context(stored_data=self.valid_data):
            step = LotseStepChooser().get_correct_step(StepSummary.name, False, ImmutableMultiDict({}))
