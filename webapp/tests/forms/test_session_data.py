...
class TestSerializeSessionData:
    ...
    @pytest.fixture(autouse=True)
    def mocking_encrypt_compress(self, monkeypatch, app):
        from zlib import compress
        from app.crypto.encryption import encrypt
        self.app = app
        self.encrypt_mock = MagicMock(wraps=encrypt)
        monkeypatch.setattr("app.forms.session_data.encrypt", self.encrypt_mock)
        self.compress_mock = MagicMock(wraps=compress)
        monkeypatch.setattr("app.forms.session_data.zlib.compress", self.compress_mock)
        yield
    ...
    def test_serialization_should_encrypt_and_compress(self):
        ...
        self.encrypt_mock.assert_called()
        self.compress_mock.assert_called()
...
class TestDeserializeSessionData:
    ...
    def mocking_encrypt_compress(self, monkeypatch, app):
        ...
        self.app = app
        self.decrypt_mock = MagicMock(side_effect=InvalidToken)
        monkeypatch.setattr("app.forms.session_data.decrypt", self.decrypt_mock)
        self.mocked_time = MagicMock()
        monkeypatch.setattr("time.time", self.mocked_time)
        self.log_fun = MagicMock()
        monkeypatch.setattr("app.forms.session_data.logger.warn", self.log_fun)
        yield
    ...
    def test_if_ttl_smaller_than_passed_time_then_return_original_data(self):
        ...
        self.mocked_time.return_value = creation_time
        serialized_data = serialize_session_data(original_data)

        self.mocked_time.return_value = passed_time
    ...
    def test_if_session_data_empty_do_not_log_error(self):
        ...
        assert {} == deserialized_data
        self.log_fun.assert_not_called()
